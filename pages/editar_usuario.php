...
if(isset($_POST['enviar'])) {
    ...
    if(count($erro) == 0) {
        $sql_code = "UPDATE usuarios 
                     SET nome = ?, email = ?, creditos = ?, admin = ?
                     WHERE id = $id";

        $insert_values = [$nome, $email, $creditos, $admin];

        if(!empty($senha)) {
            $senha = password_hash($senha, PASSWORD_DEFAULT);

            $sql_code = "UPDATE usuarios 
                         SET nome = ?, email = ?, senha = ?, creditos = ?, admin = ?
                         WHERE id = $id";
            
            $insert_values = [$nome, $email, $senha, $creditos, $admin];
        }

        $sql = $pdo->prepare($sql_code);
        $criado = $sql->execute($insert_values);
...
$sql_query = $pdo->query("SELECT * FROM usuarios WHERE id = $id");
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-4">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="creditos" value="<?php echo $usuario['creditos'] ?>" class="form-control">
                                </div>  
                            </div>
